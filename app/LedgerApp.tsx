...
import {
  ...
  ReceiptText, RotateCcw, Search, Settings, ShieldCheck, Sun, Trash2, WalletCards, X, Zap,
} from "lucide-react";
import { db, exportBackup, exportTransactionsCsv, importBackup, initializeDatabase, loadSnapshot, resetDatabase } from "@/lib/db";
import {
...
export function LedgerApp() {
  ...
  const refresh = useCallback(async () => {
    try { setData(await loadSnapshot()); setError(""); } catch (reason) { setError(reason instanceof Error ? reason.message : "读取本地数据失败"); }
  }, []);
  useEffect(() => {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      <header className="topbar">
        ...
        <div className="top-actions"><button className="icon-btn" aria-label="切换主题" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>{theme === "light" ? <Moon /> : <Sun />}</button><button className="avatar" onClick={() => navigate("settings")}>青</button></div>
      </header>
      ...
      {view === "settings" && <SettingsView data={data} theme={theme} setTheme={setTheme} onRefresh={refresh} setToast={setToast} />}
    </main>
...
function Transactions({ data, onRefresh, onEdit, setToast }: { data: LedgerSnapshot; onRefresh: () => Promise<void>; onEdit: (tx: LedgerTransaction) => void; setToast: (s: string) => void }) {
  ...
  const remove = async (id: string) => { if (pendingDelete !== id) { setPendingDelete(id); setToast("再次点击删除以确认"); return; } await db.transactions.delete(id); setPendingDelete(null); await onRefresh(); setToast("记录已删除"); };
...
function QuickEntry({ data, editing, onDone, onCancel }: { data: LedgerSnapshot; editing: LedgerTransaction | null; onDone: () => Promise<void>; onCancel: () => void }) {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (attachment) { const id = uid("attachment"); await db.attachments.add({ id, name: attachment.name, type: attachment.type, size: attachment.size, blob: attachment }); attachmentIds.push(id); }
...
function BudgetView({ data, metrics: m, onRefresh, setToast }: { data: LedgerSnapshot; metrics: ReturnType<typeof deriveMetrics>; onRefresh: () => Promise<void>; setToast: (s: string) => void }) {
  ...
  const saveCap = async (value: string) => { const cents = asCents(Number(value)); if (cents < 0) return; await db.budgets.update("main", { weeklyCapCents: cents }); await onRefresh(); setToast("预算上限已更新"); };
...
function SalaryView({ data, metrics: m, onRefresh, setToast }: { data: LedgerSnapshot; metrics: ReturnType<typeof deriveMetrics>; onRefresh: () => Promise<void>; setToast: (s: string) => void }) {
  ...
  const toggle = async (id: string, worked: boolean) => { await db.attendance.update(id, { status: worked ? "worked" : "off", earnedCents: worked ? plan.dailyRateCents : 0 }); await onRefresh(); };
  const confirmPaid = async () => { if (!m.salary.receivableCents) return; await db.transactions.add({ id: uid("salary"), type: "salary_payment", status: "posted", amountCents: m.salary.receivableCents, date: TODAY, time: new Date().toTimeString().slice(0, 5), accountId: data.accounts[0].id, categoryId: "income-工资-暑假工工资", merchant: plan.employer, note: "手动确认工资到账", countsTowardBudget: false, rigid: false, reimbursable: false, tags: ["工资"], attachmentIds: [], affectsBalance: true, createdAt: new Date().toISOString() }); await onRefresh(); setToast("工资已转为到账收入"); };
...
function SettingsView({ data, theme, setTheme, onRefresh, setToast }: { data: LedgerSnapshot; theme: string; setTheme: (t: "light" | "dark") => void; onRefresh: () => Promise<void>; setToast: (s: string) => void }) {
  ...
  return <div className="settings-grid"><section className="settings-section"><h2>预算与安全线</h2><SettingRow label="安全线范围" hint="决定哪些近期分期计入锁定资金"><select value={data.budget.safetyMode} onChange={async (e) => { await db.budgets.update("main", { safetyMode: e.target.value as typeof data.budget.safetyMode }); await onRefresh(); }}><option value="30d">未来30天</option><option value="60d">未来60天</option><option value="school">计算到开学</option><option value="custom">自定义金额</option></select></SettingRow><SettingRow label="每周结余" hint="决定结余是否滚入下周"><select value={data.budget.rolloverMode} onChange={async (e) => { await db.budgets.update("main", { rolloverMode: e.target.value as "rollover" | "reset" }); await onRefresh(); }}><option value="reset">每周清零</option><option value="rollover">结余滚存</option></select></SettingRow><SettingRow label="开学日期" hint="用于安全线和未来预测"><input type="date" value={data.budget.schoolDate} onChange={async (e) => { await db.budgets.update("main", { schoolDate: e.target.value as LocalDate }); await onRefresh(); }} /></SettingRow></section><section className="settings-section"><h2>显示与隐私</h2><SettingRow label="界面主题" hint="主题偏好仅保存在本机"><div className="theme-toggle"><button className={theme === "light" ? "active" : ""} onClick={() => setTheme("light")}><Sun />浅色</button><button className={theme === "dark" ? "active" : ""} onClick={() => setTheme("dark")}><Moon />深色</button></div></SettingRow><SettingRow label="本地数据" hint="账本不会上传服务器"><span className="local-badge"><ShieldCheck />IndexedDB 已启用</span></SettingRow></section><section className="settings-section wide"><h2>备份与恢复</h2><div className="backup-actions"><button onClick={exportJson}><Download />导出完整 JSON</button><button onClick={() => downloadText(`青蓝流水-${TODAY}.csv`, exportTransactionsCsv(data.transactions, data.accounts, data.categories), "text/csv;charset=utf-8")}><Download />导出流水 CSV</button><label className="button-label"><FileUp />导入 JSON<input type="file" accept="application/json" onChange={(e) => previewImport(e.target.files?.[0])} /></label></div>{importInfo && <div className="import-preview"><div><strong>导入预览</strong><span>{importInfo.accounts} 个账户 · {importInfo.transactions} 条流水</span><p>合并会按 ID 更新同名记录；覆盖会先清除当前账本，请确保已导出备份。</p></div><button onClick={() => runImport("merge")}>合并导入</button><button className="danger" onClick={() => runImport("replace")}>覆盖恢复</button></div>}</section><section className="settings-section wide danger-zone"><h2>危险操作</h2><p>输入“清空数据”后可恢复首次示例数据。此操作会删除当前设备上的所有账目与凭证。</p><div className="inline-form"><input value={clearText} onChange={(e) => setClearText(e.target.value)} placeholder="输入：清空数据" /><button className="danger solid" disabled={clearText !== "清空数据"} onClick={async () => { await resetDatabase(); setClearText(""); await onRefresh(); setToast("已恢复示例数据"); }}><RotateCcw />清空并恢复示例</button></div></section></div>;
}
